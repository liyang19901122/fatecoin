...
static const unsigned short int MAX_TX_DATA      = 500;
static const unsigned short int MAX_TX_DATA_FROM = 40;
static const unsigned short int MAX_TX_DATA_SUBJ = 40;
static const unsigned short int MAX_TX_DATA_MSG  = 140;
...
static const int64 MAX_MONEY = 66666666 * COIN;	//84000000 * COIN;
inline bool MoneyRange(int64 nValue) { return (nValue >= 0 && nValue <= MAX_MONEY); }
/** Coinbase transaction outputs can only be spent after this number of new blocks (network rule) */
static const int COINBASE_MATURITY = 60;	//100;
...
// Fatecoin: calculate Nfactor using timestamp
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, int nHeight = 0);
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, int nHeight = 0);
...
public:
    ...
    bool ReadFromDisk(const CDiskBlockPos &pos)
    {
        ...
        // Check the header
        if (!CheckProofOfWork(GetHash(), nBits))	//if (!CheckProofOfWork(GetPoWHash(), nBits))
    ...
    void print() const
    {
        printf("CBlock(hash=%s, input=%s, PoW=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nBits=%08x, nNonce=%u, cBits=%08x, vtx=%"PRIszu")\n",
            GetHash().ToString().c_str(),
            HexStr(BEGIN(nVersion),BEGIN(nVersion)+80,false).c_str(),
            GetHash().ToString().c_str(),	//GetPoWHash().ToString().c_str(),
            nVersion,
            hashPrevBlock.ToString().c_str(),
            hashMerkleRoot.ToString().c_str(),
            nTime, nBits, nNonce, cBits, 
            vtx.size());
