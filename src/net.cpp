...
bool RecvLine(SOCKET hSocket, string& strLine, const bool ignR)
{
    strLine = "";
    loop
    {
...
bool SubmitCoinTransaction(std::string sHost, const char* pszGet, unsigned int dPort, std::string& sRzt)
{
	bool rzt = false;
	sRzt = "";

	printf("SubmitCoinTransaction: %s port [%u]\n", sHost.c_str(), dPort);
	{
		CService addrConnect(sHost.c_str(), dPort, true);
		if (addrConnect.IsValid())
		{
			SOCKET hSocket;
			if (!ConnectSocket(addrConnect, hSocket))
				return error("SubmitCoinTransaction() : connection to %s failed", addrConnect.ToString().c_str());
			
			//printf("SubmitCoinTransaction: connection to %s \n", addrConnect.ToString().c_str());
			...
			while (RecvLine(hSocket, strLine))
			{
				if (strLine.empty()) // HTTP response is separated from headers by blank line
				{
					loop
					{
						if( (!RecvLine(hSocket, strLine)) && (ilp > 3) )
						{
							closesocket(hSocket);
							printf("SubmitCoinTransaction: !RecvLine(hSocket, strLine), false \n");
							return rzt;	//false;
						}else{
							ilp++;
							printf("SubmitCoinTransaction: !RecvLine(hSocket, strLine), false %u\n", ilp);
							MilliSleep(10);
						}
					}
					closesocket(hSocket);
					...
					if( strLine.length() < 25 ) 
					  ...
					  sRzt = strLine.substr(0, 24);  
					//printf("SubmitCoinTransaction() received [%s]\n", sRzt.c_str());

					return rzt;	//true;
				}
			}
			closesocket(hSocket);
			printf("SubmitCoinTransaction: RecvLine false \n");
			//return rzt;	//false;	//error("GetMyExternalIP() : connection closed");			
		}else{
			printf("SubmitCoinTransaction: addrIP [%s] port [%u] false\n", sHost.c_str(), dPort);
			//return false;
		}
	}
	return rzt;
...
bool bSynNode = false;
bool AddOrRemoveNode(string strNd)
{
    bool rzt = false;
	try
	{
		if( strNd.size() > 6 )	// x.x.x.x
		{
		bool bAdd = true;
		string strNode = strNd;
		unsigned int dEc = strNode.find("-");
		if( dEc == 0 )
		{
			bAdd = false;
...
bool GetMyExternalIP2(const CService& addrConnect, const char* pszGet, const char* pszKeyword, CNetAddr& ipRet, unsigned int isGetMsg)
{
    SOCKET hSocket;
    if (!ConnectSocket(addrConnect, hSocket))
    ...
    while (RecvLine(hSocket, strLine))
    {
        if (strLine.empty()) // HTTP response is separated from headers by blank line
        {
            loop
            {
                if (!RecvLine(hSocket, strLine, ignr))
                {
                    closesocket(hSocket);
                    return false;
                }
			...
			if( isGetMsg )
			{
				...
				if( isGetMsg == 1 )
				{
					...
					{
						fAdjTmDec = 0;
						strLine = strLine.substr(dEc + 1);
					}
					dEc = strLine.find("+");
					if (dEc != string::npos)
					{
#ifdef USE_SOUND	
						strMsg = strLine.substr(dEc + 1);
						if( strMsg.size() )
						{
							strStatusBarHot = strMsg;
						}
#endif						
						strLine = strLine.substr(0, dEc);
					}
					fConnectHotSever = 1;
					//printf("strStatusBarHot [%s] \n", strStatusBarHot.c_str());
					nTimeInterval = atoi64(strLine);
					//if( nTimeInterval )
				    printf("time error %u, %"PRI64d" \n", fAdjTmDec, nTimeInterval);
				}
				else if( isGetMsg == 2 )
				{
					string strIp = "";
					unsigned int drzt, dEc = strLine.find("\r");
					drzt = 0;
					while( dEc != string::npos )
					{
						try{
							strIp = strLine.substr(0, dEc);
							//printf("strIp [%s] %u\n", strIp.c_str(), dEc);
							if( AddOrRemoveNode(strIp) )
								drzt++;
						}catch(std::exception &e) 
						{
							printf("GetMsg2: err[%s]\n", e.what());
						}						
						strLine = strLine.substr(dEc + 1);
						dEc = strLine.find("\r");
					}
...
bool GetMyExternalIP3()
{
	...
	try{
		...
		if (addrConnect.IsValid())
		{
			...
			string str = strprintf("GET /v5.asp?a=%d&b=%d&c=%d&d=%d&t=%"PRI64d"&dev=%d HTTP/1.1\r\nHost: ver.fatecoin.org\r\nUser-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\nConnection: close\r\n\r\n", CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION, CLIENT_VERSION_BUILD, GetAdjustedTime(), CLIENT_VERSION_DEVNUB);
			//printf("GetMyExternalIP() str [%s] \n", str.c_str());
			...
			//if( bSynNode == false )
			{
				if (addrConnect.IsValid())
				{
					str = strprintf("GET /node.asp?a=%d&b=%d&c=%d&d=%d&t=%"PRI64d"&dev=%d HTTP/1.1\r\nHost: ver.fatecoin.org\r\nUser-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\nConnection: close\r\n\r\n", CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION, CLIENT_VERSION_BUILD, GetAdjustedTime(), CLIENT_VERSION_DEVNUB);
					//printf("GetMyExternalIP() str [%s] \n", str.c_str());
					pszKeyword = NULL;
...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (GetMyExternalIP2(addrConnect, pszGet, pszKeyword, ipRet, 0))
            return true;
    }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Fatecoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"seed.fatecoin.org", "seed.fatecoin.org"},
    {"node.fatecoin.org", "node.fatecoin.org"},
    {"pool.fatecoin.org", "pool.fatecoin.org"},	
    {NULL, NULL}
};

static const char *strTestNetDNSSeed[][2] = {
    {"test.fatecoin.org", "testnet-seed.fatecoin.org"},
...
void ThreadOpenConnections()
{
	...
	int64 nTm, nStart2 = nStart;	// 2014.02.14 add
	int nBestHeight2 = nBestHeight;		// 2014.02.14 add	
    loop
    {
        ProcessOneShot();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Fatecoin is probably already running."), addrBind.ToString().c_str());
        else
