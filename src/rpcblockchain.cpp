...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    int nShift = (blockindex->cBits >> 24) & 0xff;

    double dDiff =
        (double)0x0000ffff / (double)(blockindex->cBits & 0x00ffffff);
...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex)
{
	...
	result.push_back(Pair("Value", block.vtx[0].vout[0].nValue));	// 2014.01.08 add
    ...
    result.push_back(Pair("bits", HexBits(block.cBits)));
...
Value getblockinfo(const Array& params, bool fHelp)
{
	...
	string str = pblockindex->phashBlock->GetHex();
	if( str.size() )
	{

		std::string strHash = str;
		uint256 hash(strHash);
