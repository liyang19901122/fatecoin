        ...
        cachedNumBlocks(0)
{
	...
	columns << QString() << tr("Date") << tr("Type") << tr("Address") << tr("Amount") << tr("Message");
	...
	if( GetBoolArg("-autosubmit", false) )
	  timer->start(MODEL_UPDATE_DELAY + MODEL_UPDATE_DELAY);
    else
...
QString TransactionTableModel::formatTxData(const TransactionRecord *wtx) const
{
	...
	//if( wtx.nVersion > 1 )
    {
        ...
        if(mi != wallet->mapWallet.end())
            str = TransactionDesc::GetTxMessage(mi->second);
    }
    return str;
}
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        } else if (role == Qt::ToolTipRole)
        {
            ...
            case hotData:
                return tr("Transaction's Message.");				
            }
        }
    }
    return QVariant();
