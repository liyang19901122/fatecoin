...
uint256 hashGenesisBlock("0x00000acd7d7cd7b01f5a800486a0b93b33ad087c7fdebac2f9765730cdd7e7dc");
//static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fatecoin: starting difficulty is 1 / 2^12
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);
static int64 nSubsidyForDev = 0.00140213 * COIN;
int64 nChainStartTime = 1392266666;
...
const string strMessageMagic = "Fatecoin Signed Message:\n";
...
{
    // Fatecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (nVersion > 2 || nVersion < 1) {	//if (nVersion > CTransaction::CURRENT_VERSION || nVersion < 1) {
        strReason = "version";
        return false;
    }
	
	// Disallow large transaction data
	if (fatedata.length() > MAX_TX_DATA)
			return false;	
...
// Fatecoin: increasing Nfactor gradually
...
static const unsigned int nMaxCoinInt = 66;
int64 nMaxCoinPerBlock = nMaxCoinInt * COIN;
static const unsigned int nCpuProofOfWork = bnProofOfWorkLimit.GetCompact();
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if( nHeight > 86400 )		// one day = 2 * 60 * 24 = 2880, 2880 * 30 = 86400
	{
		iMax = nMaxCoinInt;		// after 30 days, 66 coin
		iMin = 3;
	}
	else{	// 1~30 days, 100 coin per block
		iMax = 100;
		iMin = 6;
	}
	nMaxCoinPerBlock = iMax * COIN;
    i = getrandint(iMin, iMax);	// min, max
	nSubsidy = i * COIN;
	
	// Subsidy is cut in half every 172800 blocks(60 days), which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 172800)  + nFees; // Fatecoin: 172800 blocks in ~4 years
	nMaxCoinPerBlock >>= (nHeight / 172800);
	if( nMaxCoinPerBlock < nSubsidy ) nMaxCoinPerBlock = nSubsidy;
	
    return nSubsidy;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Fatecoin: 3 days
static const int64 nTargetSpacing = 60; // Fatecoin: 2 minutes
...
double GetDifficultyByBits(unsigned int cBits)
{
    int nShift = (cBits >> 24) & 0xff;
    double dDiff =
        (double)0x0000ffff / (double)(cBits & 0x00ffffff);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        ...
        return pindexLast->cBits;
    }

    // Fatecoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    // Retarget
    CBigNum bnNew;
    bnNew.SetCompact(pindexLast->cBits);
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
    ...
    // Check range
	if (nBits <= 0 || bnTarget > bnProofOfWorkLimit)		
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    int64 iCoin = GetBlockValue(pindex->nHeight, nFees);
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
	...
	if (!fIsInitialDownload)
    {
        ...
        //printf("fIsInitialDownload = 0, : %d of last 100 blocks above version %d\n", nUpgraded, CBlock::CURRENT_VERSION);
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fatecoin: Special short-term limits to avoid 10,000 BDB lock limit:
    ...
    // Check proof of work matches claimed amount
    if (fCheckPOW && !CheckProofOfWork(GetHash(), nBits))	//if (fCheckPOW && !CheckProofOfWork(GetPoWHash(), nBits))
...
{
    // Fatecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b9e9aea71be83421bc883655283b16691afa135b427b82bf254dc2bf512d94a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Thu, 13 Feb 2014 12h, Fate Dev";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1392266666;
        block.nBits    = 0x1e0ffff0;
		block.cBits    = block.nBits;
        block.nNonce   = 274828;
        ...
        printf("block.hashMerkleRoot =%s, nBits=%u\n", block.hashMerkleRoot.ToString().c_str(), block.nBits);
        assert(block.hashMerkleRoot == uint256("0xe57c589ef1196a28807f3986b1961ab90faa2372afa7937061bd984fad5cd470"));
        block.print();
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
				...
				printf("strStatusBar %s\n", strStatusBar.c_str());
            }
        }
    }
	//printf("strStatusBar (%s), strFor (%s)\n", strStatusBar.c_str(), strFor.c_str());
    if (strFor == "statusbar")
	{
#ifdef USE_SOUND        
		if( strStatusBarHot.size() )
		{
			return strStatusBarHot;
		}
#endif		
		return strStatusBar;
    }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Fatecoin: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        printf("receive version message: %s: version %d, blocks=%d, us=%s, them=%s, peer=%s, devNub=%u, ch%u, aTime=%"PRI64d", gT2=%"PRI64d", AT=%"PRI64d"\n", pfrom->strSubVer.c_str(), pfrom->nVersion, pfrom->nStartingHeight, addrMe.ToString().c_str(), addrFrom.ToString().c_str(), pfrom->addr.ToString().c_str(), vDevnub, vConnetSrv, aTime, ngT2, GetAdjustedTime());
...
unsigned int static GetProofOfWork(const CBlockIndex* pindexLast, const CBlockHeader *pblock, int nHeight)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
	...
	if( nHeight )
	{
		unsigned int i = GetRandInt(nProofOfWorkLimit - 486785040);
		nProofOfWorkLimit = nProofOfWorkLimit - i;
		CBigNum bnNew;
		...
		return nProofOfWorkLimit;		
	//double nd = GetDifficultyByBits(nProofOfWorkLimit);	
	//printf("nHeight =[%u], difficulty [%0.8f], [%u]\n", pindexLast->nHeight+1, nd, nProofOfWorkLimit);			
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, int nHeight)
{
	// Create new block
	...
	if( checkStartTime == 0 )
		...
		int64 nTm = GetTime();
		if( nTm > checkStartTime )
		{
			if( (nTm - checkStartTime) > (60 * 6) )
			{
				checkStartTime = nTm;
				GetMyExternalIP3();
			}
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, int nHeight)
{
    CPubKey pubkey;
    ...
    return CreateNewBlock(scriptPubKey, nHeight);
}
...
{
	if( CheckHash(pblock) == false )
		return false;

    //// debug print
    printf("FatecoinMiner:\n");
    pblock->print();
	...
	{	
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FatecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FatecoinMiner : ProcessBlock, block not accepted");
    }
    return true;
}

void static FatecoinMiner(CWallet *pwallet)
{
    void *scratchbuf = scrypt_buffer_alloc();
	
	printf("FatecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fatecoin-miner");
    ...
    try { loop {
        ...
        auto_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, 0));
        if (!pblocktemplate.get())
            return;
        CBlock *pblock = &pblocktemplate->block;
        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        printf("Running FatecoinMiner with %"PRIszu" transactions in block (%u bytes), %u\n", pblock->vtx.size(),
		...
		//unsigned int& nBlockBits = *(unsigned int*)(pdata + 64 + 12);
...
/*			
            ...
            if (nNonceFound != (unsigned int) -1)
            {
                if (thash <= hashTarget)
                {
                    ...
                    SetThreadPriority(THREAD_PRIORITY_NORMAL);
                    CheckWork(pblock, *pwallet, reservekey);	//CheckWork(pblock.get(), *pwalletMain, reservekey);
                    SetThreadPriority(THREAD_PRIORITY_LOWEST);
                    break;
                }
			}
    ...
    {
        printf("FatecoinMiner terminated\n");
        throw;
    }
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FatecoinMiner, pwallet));
}
