...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend,
                                CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, std::string stxData, const CCoinControl* coinControl)
{
    int64 nValue = 0;
...
#endif
        try {
            wtxNew.fatedata.clear();
            wtxNew.fatedata = stxData;
        }
        catch (std::exception &e) {
            wtxNew.fatedata = stxData;
        }
    }
	//if (!wtxNew.fatedata.empty()) wtxNew.nVersion = 2;
                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, std::string stxData, const CCoinControl* coinControl)
{
    ...
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, stxData, coinControl);
}
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee, std::string stxData)
{
    ...
    string strError;
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, strError, stxData))
    {
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee, std::string stxData)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, wtxNew, fAskFee, stxData);
}
